...
function Modules() {
  ...
  const loadModulesFromSupabase = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (insertError) {
          console.error(
            "Error creating default modules:",
            insertError
          );
        } else if (
          insertData &&
      ...
      console.error(
        "Unexpected error loading modules:",
        err
      );
    } finally {
      setLoading(false);
  ...
  const saveModulesToSupabase = async (
    updatedModules
  ) => {
    ...
    try {
      ...
      console.error(
        "Unexpected error saving modules:",
        err
      );
    }
  };
        ...
        <div className="module-selector">
          {modules.map(
            (module, index) => (
              <button
                key={module.id}
                className={`module-btn ${selectedModule ===
                  index
                  ? "active"
                  : ""}`}
                onClick={() =>
